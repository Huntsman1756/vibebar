...
import "./App.css";
import { createRefreshController, openFullDashboard, POPOVER_OPENED_EVENT, shouldAutoRefreshOnMount, type RefreshController, type RefreshFreshness } from "./appRuntime";
...
import { rankByPrimary, resolveCost, type CostSummary, type LocalPriceTable } from "./efficiency";
import {
...
function historyCostLabel(cost: CostSummary) {
  return cost.kind === "reported" ? "Reported cost" : cost.kind === "estimated" ? "Estimated cost" : "Cost unavailable";
}

function historyCostDetail(cost: CostSummary) {
...
function HistoryCostCell({ rows, priceTable }: { rows: UsageHistoryRow[]; priceTable: LocalPriceTable }) {
  const cost = resolveCost(rows, priceTable);
  return <><strong>{formatObservedCost(cost.amountMicrousd) ?? "Unavailable"}</strong><small>{historyCostLabel(cost)} · {historyCostDetail(cost)}</small></>;
}
...
function Progress({ value, label, tone = "mint" }: { value: number; label: string; tone?: "mint" | "violet" | "amber" }) {
  const boundedValue = Math.max(0, Math.min(value, 100));
  return <div className={`progress ${tone}`} role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={boundedValue}><span style={{ width: `${boundedValue}%` }} /></div>;
}
...
function WindowMeter({ window }: { window: QuotaWindow }) {
  return <div className="window-meter">
    ...
    <Progress value={window.usedPercent} label={`${window.label} capacity used`} tone={window.usedPercent > 80 ? "amber" : "violet"} />
...
function QuotaMeter({ quota }: { quota: ModelQuota }) {
  ...
  return <div className="quota-row">
    ...
    {metered ? <div className="quota-progress"><Progress value={quota.usedPercent ?? 0} label={`${quota.label} ${quota.periodLabel} capacity used`} tone={limited ? "amber" : "violet"} /><small>{quotaPercent(quota.remainingPercent)} left</small></div> : <small className="quota-unavailable">{localUnmeasurableQuotaLabel}</small>}
  </div>;
}
...
export function AgentUsagePanel({ usage }: { usage: typeof demoSnapshot.agentUsage }) {
  return <section className="agent-panel">
    <div className="section-head compact"><div><p className="eyebrow">AGENTS / ROLES</p><h2>Who spent the tokens</h2></div><span>90 days retained</span></div>
...
function CompactProviderCard({ provider, rows }: { provider: ProviderSnapshot; rows: UsageHistoryRow[] }) {
  ...
  const providerMetrics = diagnosticForProvider(providerRows, provider.id);
  const providerModels = aggregateHistoryByProvider(providerRows);
  ...
  return <article className={`compact-provider ${provider.status === "error" ? "provider-error" : ""}`}>
    ...
    {providerRows.length === 0 ? <p className="compact-empty">No traffic in the selected period.</p> : <><div className="compact-provider-total"><strong>{formatTokens(providerMetrics.primaryTokens)}</strong><span>Primary traffic · {formatTokens(providerMetrics.reasoningTokens)} reasoning · {formatRatioPercent(providerMetrics.cacheReuse)} cache reuse</span></div><div className="compact-provider-signals"><span>Cache reuse <strong>{formatRatioPercent(providerMetrics.cacheReuse)}</strong></span><span>Reasoning <strong>{formatTokens(providerMetrics.reasoningTokens)}</strong></span><span>Top model <strong>{topModel?.model ?? "—"}</strong></span></div><div className="compact-model-list">{providerModels.slice(0, 3).map((model) => { const sourceModel = provider.models.find((item) => item.model === model.model); const quota = sourceModel?.quotaWindows.find((item) => hasCompleteQuotaMeter(item)) ?? sourceModel?.quotaWindows[0]; return <div className="compact-model" key={model.model}><div><strong>{model.model}</strong><small>{formatTokens(model.observedTokens)} observed</small><small>{formatTokens(model.billableTokens)} Primary (input + output)</small><small>{formatTokens(model.inputTokens)} input · {formatTokens(model.outputTokens)} output · {formatTokens(model.reasoningTokens)} reasoning · {formatTokens(model.cacheReadTokens)} cache read · {formatTokens(model.cacheWriteTokens)} cache write</small></div><span>{quota ? hasCompleteQuotaMeter(quota) ? `${quotaPercent(quota.remainingPercent)} left` : localUnmeasurableQuotaLabel : "Sin cuota conocida"}</span></div>; })}</div></>}
  </article>;
}
...
function HistoryChart({ rows, providerLabels, providerTotals, providerTones }: {
  ...
  if (series.length === 0) {
    return <HistoryState title="No daily usage in this period" copy="Try a wider range to see provider activity across the local 90-day series." />;
  }
...
function ProviderHistoryTable({ summaries, providerLabels, rows, priceTable }: { summaries: ProviderHistorySummary[]; providerLabels: Map<string, string>; rows: UsageHistoryRow[]; priceTable: LocalPriceTable }) {
  return <article className="history-table-card">
    ...
    <table className="history-table">
      ...
      <thead><tr><th>Provider / model</th><th>Source fidelity</th><th>Observed total</th><th>Primary (input + output)</th><th>Input</th><th>Output</th><th>Reasoning</th><th>Cache read</th><th>Cache write</th><th>Msgs</th><th>Sessions</th><th>Cost</th></tr></thead>
      <tbody>
        {summaries.map((summary) => <tr key={`${summary.provider}-${summary.model}`}>
          ...
          <td><HistoryCostCell rows={rows.filter((row) => row.provider === summary.provider && row.model === summary.model)} priceTable={priceTable} /></td>
        </tr>)}
      </tbody>
    </table>
  </article>;
...
function AgentHistoryTable({ summaries, providerLabels, rows, priceTable }: { summaries: AgentHistorySummary[]; providerLabels: Map<string, string>; rows: UsageHistoryRow[]; priceTable: LocalPriceTable }) {
  return <article className="history-table-card">
    ...
    <table className="history-table">
      ...
      <thead><tr><th>Agent</th><th>Provider / model</th><th>Repository</th><th>Source fidelity</th><th>Observed total</th><th>Primary (input + output)</th><th>Input</th><th>Output</th><th>Reasoning</th><th>Cache read</th><th>Cache write</th><th>Sessions</th><th>Cost</th></tr></thead>
      <tbody>
        {summaries.map((summary) => <tr key={`${summary.agent}-${summary.provider}-${summary.model}-${summary.repository}`}>
          ...
          <td>{formatTokens(summary.cacheWriteTokens)}</td>
          <td>{compact.format(summary.sessionCount)}</td>
          <td><HistoryCostCell rows={rows.filter((row) => row.agent === summary.agent && row.provider === summary.provider && row.model === summary.model && row.repository === summary.repository)} priceTable={priceTable} /></td>
        </tr>)}
      </tbody>
    </table>
  </article>;
...
export function HistoryPanel({ snapshot, providerLabels, range, onRangeChange, priceTable }: { snapshot: DashboardSnapshot | null; providerLabels: Map<string, string>; range: HistoryRange; onRangeChange: (value: HistoryRange) => void; priceTable: LocalPriceTable }) {
  ...
  const cost = useMemo(() => resolveCost(rows, priceTable), [priceTable, rows]);
  ...
  return <section className="history-panel">
    ...
    {!history ? <HistoryState title="Loading historical usage" copy="Refreshing local provider and repository history from the current snapshot." /> : unavailable ? <HistoryState title="Historical usage unavailable" copy="The local snapshot has not produced bounded history rows yet, so VibeBar is avoiding invented zero values." /> : empty ? <HistoryState title="No usage in this period" copy={`There are no billable or cache rows for ${historyRangeLabel(range).toLowerCase()}. Try a wider window or wait for the next snapshot.`} /> : <>
      ...
      <div className="history-summary-grid">
        ...
        <div className="history-summary-card"><span>{historyCostLabel(cost)}</span><strong>{formatObservedCost(cost.amountMicrousd) ?? "Unavailable"}</strong><small>{historyCostDetail(cost)}</small></div>
      </div>
      ...
      <div className="history-table-grid">
        <ProviderHistoryTable summaries={providers} providerLabels={providerLabels} rows={rows} priceTable={priceTable} />
        <RepositoryHistoryTable summaries={repositories} />
        <AgentHistoryTable summaries={agents} providerLabels={providerLabels} rows={rows} priceTable={priceTable} />
      </div>
    </>}
  </section>;
}
...
export function PopoverDashboard({
  ...
  priceTable,
  freshness = preview ? "preview" : error ? (snapshot ? "stale" : "unavailable") : "fresh",
}: {
  ...
  const baselineRows = useMemo(() => history ? previousComparableHistoryRows(history.rows, historyRange, localToday) : [], [history, historyRange, localToday]);
  ...
  const openFull = async () => {
    setOpenError(await openFullDashboard(() => invoke("open_full_dashboard")));
  };
  const freshnessMessage = freshness === "preview"
    ? "Browser preview — sample data."
    : freshness === "stale"
      ? "Showing the last successful snapshot; live data may be out of date."
  ...
  return <main className="app-shell popover-shell">
    ...
    <div className="popover-content">
      ...
      <p className={`freshness-status ${freshness}`} role="status" aria-live="polite">{freshnessMessage}</p>
      ...
      {error ? <p className="compact-error" role="status" aria-live="polite">{error}</p> : null}
      {openError ? <p className="compact-error" role="status" aria-live="polite">{openError}</p> : null}
      <button className="open-dashboard" onClick={() => void openFull()}>Open full dashboard <span>↗</span></button>
    </div>
  </main>;
}
...
export function DashboardStorageStatus() {
  return <aside className="ingest-card"><p className="eyebrow">LOCAL CONTRACT</p><h3>Append-only telemetry</h3><p>VibeBar never needs provider secrets. Your orchestrator sends bounded usage and review outcomes to local storage.</p><div className="path"><span>STORAGE STATUS</span><code>Stored locally on this device · location hidden</code></div><ul><li><i />Idempotent event IDs</li><li><i />64 KiB per event ceiling</li><li><i />Malformed lines become diagnostics</li></ul></aside>;
}
...
export function DashboardOverview({
  ...
  onHistoryRangeChange: (value: HistoryRange) => void;
  priceTable: LocalPriceTable;
  onSavePriceTable: (table: LocalPriceTable) => void;
  onClearPriceTable: () => void;
}) {
  ...
  const previousComparableRows = useMemo(() => previousComparableHistoryRows(historyRows, historyRange, localToday), [historyRows, historyRange, localToday]);
  ...
  return <section className="overview-panel">
    <div className="section-head"><div><p className="eyebrow">OVERVIEW</p><h2>Spend, explained</h2></div><HistoryRangeButtons value={historyRange} onChange={onHistoryRangeChange} /></div>
    <DecisionStrip rows={selectedHistoryRows} baselineRows={previousComparableRows} rangeLabel={historyRangeLabel(historyRange)} priceTable={priceTable} />
    <EfficiencyPanel rows={selectedHistoryRows} baselineRows={previousComparableRows} priceTable={priceTable} />
    <PriceTableEditor modelKeys={modelKeys} priceTable={priceTable} onSave={onSavePriceTable} onClear={onClearPriceTable} />
  </section>;
}
...
function App() {
  ...
  const [freshness, setFreshness] = useState<RefreshFreshness>("unavailable");
  ...
  const refreshControllerRef = useRef<RefreshController<DashboardSnapshot> | null>(null);
  if (!refreshControllerRef.current) {
    refreshControllerRef.current = createRefreshController({ isTauriRuntime: tauriRuntime, demoSnapshot });
  }

  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const next = refreshControllerRef.current!.transition({ kind: "success", snapshot: await invoke<DashboardSnapshot>("dashboard_snapshot") });
      setSnapshot(next.snapshot);
      setPreview(next.preview);
      setFreshness(next.freshness);
      setError(next.error);
    } catch (reason) {
      const next = refreshControllerRef.current!.transition({ kind: "failure", error: String(reason) });
      setSnapshot(next.snapshot);
      setPreview(next.preview);
      setFreshness(next.freshness);
      setError(next.error);
  ...
  if (isPopover) {
    return <PopoverDashboard snapshot={snapshot} providers={providers} loading={loading} preview={preview} error={error} refresh={refresh} providerLabels={providerLabels} historyRange={historyRange} onHistoryRangeChange={setHistoryRange} priceTable={priceTable} freshness={freshness} />;
  }
  ...
  return <main className="app-shell">
    ...
    <DashboardOverview snapshot={snapshot} historyRange={historyRange} onHistoryRangeChange={setHistoryRange} priceTable={priceTable} onSavePriceTable={savePriceTable} onClearPriceTable={clearPriceTable} />
    ...
    <HistoryPanel snapshot={snapshot} providerLabels={providerLabels} range={historyRange} onRangeChange={setHistoryRange} priceTable={priceTable} />
